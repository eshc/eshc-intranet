...
def voting_open_sessions():
    ...
    return ApplicationSession.objects.filter(
        voting_open_time__lte=t, voting_close_time__gte=t
    )
...
class Applicant(models.Model):
    session = models.ForeignKey(
        ApplicationSession, on_delete=CASCADE, verbose_name="Application session"
    )
    first_name = models.CharField(verbose_name="First name", max_length=30)
    last_name = models.CharField(verbose_name="Last name", max_length=150)
    preferred_name = models.CharField(
        verbose_name="Preferred name", max_length=30, blank=True
    )
    email = models.EmailField(verbose_name="Email address")
    phone_number = models.CharField(
        verbose_name="Phone number", max_length=15, blank=True
    )
    is_past_applicant = models.BooleanField(
        verbose_name="Past applicant", default=False
    )
    verified_past_applicant = models.BooleanField(
        verbose_name="Verified past applicant", default=False
    )
    confidential_note = models.TextField(
        verbose_name="Confidential information", max_length=1000, blank=True
    )
    app_team_note = models.TextField(
        verbose_name="Applications team notes (invisible to applicant)",
        max_length=1000,
        blank=True,
    )
