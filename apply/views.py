...
class ApplyView(TemplateView):
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        ...
        app_session: models.ApplicationSession = get_object_or_404(
        ...
        if (
            models.Applicant.objects.filter(
                session=app_session, email=data["email"]
            ).count()
            > 0
        ):
            mistakes.append(
...
"""
        ...
        if len(mistakes) > 0:
            ctx["error_message"] = (
                "Please correct some of your answers below. Mistakes in fields: "
                + ", ".join(mistakes)
            )
...
def get_answers(applicant: models.Applicant):
    ...
    for q in qs:
        try:
            a: models.ApplicationAnswer = models.ApplicationAnswer.objects.get(
                applicant=applicant, question=q
            )
...
class VoteView(TemplateView):
    ...
    def get(self, request: HttpRequest, *args, **kwargs):
        try:
            ...
            if session.is_voting_open:
                ...
                next_applicant: models.Applicant = find_applicant(
                    ctx["session"], member
                )
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        ...
        if not session.is_voting_open:
            ...
            return self.render_to_response(ctx)
        member: User = request.user
        applicant: models.Applicant = models.Applicant.objects.get(
        ...
        if applicant.session != session:
            ...
            return self.render_to_response(ctx)
        if (
            models.ApplicationVote.objects.filter(
                applicant=applicant, voting_member=member
            ).count()
            != 0
        ):
            ctx["error_message"] = "You have already voted for applicant #%d!" % (
                applicant.pk,
            )
            return self.render_to_response(ctx)
