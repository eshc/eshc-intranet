...
class CensusResultsView(TemplateView):
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        ...
        app_session: models.CensusSession = get_object_or_404(
            models.CensusSession, pk=kwargs["session_id"]
        )
...
class CensusView(TemplateView):
    ...
    def get(self, request: HttpRequest, *args, **kwargs):
        ...
        if value is True:
            # Cookie is not set
            return render(
                request, self.already_filled_template_name, ctx, self.content_type
            )
        else:
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        ...
        for q in questions:
            ...
            if (
                q.question_type == models.QuestionType.LongText.name
                or q.question_type == models.QuestionType.ShortText.name
            ):
                r = models.CensusResponse.objects.create(
                    session=app_session, question=q, answer_text=data["answers"][q.pk]
                )
                ...
                try:
                    ...
                    r = models.CensusResponse.objects.create(
                        session=app_session, question=q, answer_choice=i
                    )
                except ValueError:
        ...
        if len(mistakes) > 0:
            ...
            r.save()
            response = render(
                request, self.success_template_name, ctx, self.content_type
            )
