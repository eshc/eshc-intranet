...
class CensusSession(models.Model):
    ...
    census_description = models.TextField(
        verbose_name="Census Description", default="", blank=True
    )
...
class CensusQuestion(OrderedModel):
    ...
    def get_aggregated_responses(self):
        ...
        for key in range(len(self.options_array())):
            answers.append(
                (
                    self.options_array()[key],
                    CensusResponse.objects.filter(
                        session=self.session, question=self, answer_choice=key
                    ).count(),
                )
            )
        return answers
...
class CensusResponse(models.Model):
    ...
    session = models.ForeignKey(
        CensusSession, on_delete=CASCADE, verbose_name="Census session"
    )
    ...
    def get_answer_display(self):
        if (
            self.question.question_type == QuestionType.LongText
            or self.question.question_type == QuestionType.ShortText
        ):
            return self.answer_text
        elif (
            self.question.question_type == QuestionType.SingleChoice
            or self.question.question_type == QuestionType.MultipleChoice
        ):
