...
def find_lease_for_profile(u: Profile) -> Union[None, Lease]:
    ...
    try:
        return Lease.objects.filter(
            user=u.user, start_date__lte=now, end_date__gte=now
        ).latest("start_date")
...
class IntranetLdapSync:
    ...
    def __init__(self):
        self.connection = Connection(
            ...
            password=LDAP_SERVER_AUTH_PASSWORD,
            auto_bind=True,
        )
    ...
    def __unsync_intranet_user(self, user: User):
        ...
        """
        uid = user.pk
        u_query = "(&%s(|(uid=%s)(employeeNumber=%d)))" % (
            self.filter_member,
            user.username,
            uid,
        )
    ...
    def sync_intranet_user(self, user: User):
        ...
        if self.mock:
            print(
                " * Mocking ldap sync for user %s (%s)"
        ...
        if found_users == 0:
            ...
            if found_exusers == 1:
                ...
                assert 0 < len(rdn) < len(dn)
                if not self.mock:
                    succ = self.connection.modify_dn(
                        dn, rdn, delete_old_dn=True, new_superior=self.members_dn
                    )
                    assert succ
                else:
        ...
        for ldap_attr, mapfn in LDAP_ATTR_MAP.items():
            ...
            if real_value != ldap_value:
                ...
                if self.mock:
                    print(
                        "Updating field %s: '%s'->'%s'"
                        % (ldap_attr, ldap_value, real_value)
                    )
                num_changes += 1
        if num_changes > 0:
            if not self.mock:
    ...
    def __fill_ldap_group(self, group_cn: str, uids: Set[str]):
        ...
        if len(self.connection.response) == 0:
            self.connection.add(
                group_cn, "groupOfNames", {"member": self.empty_group_member}
            )
            self.connection.search(group_cn, "(cn=*)", BASE, attributes=["member"])
