...
def index(request):
    ...
    current_notes = [
        note if note.pub_date + datetime.timedelta(days=7) >= today else note.delete()
        for note in notes
    ]
...
def map(request):
    ...
    context = {  #'leases': leases,
        ...
        "flats_34": [
            {"flat": f, "rooms": rooms.filter(flat=f)}
            for f in flats.filter(building=34)
        ],
    }
...
def archive(request):
    ...
    oldest_gms = [
        s3.generate_presigned_url(
            "get_object", Params={"Bucket": "eshc-bucket", "Key": x}
        )
        for x in oldest_archive_keys
    ]
    year_1_gms = [
        s3.generate_presigned_url(
            "get_object", Params={"Bucket": "eshc-bucket", "Key": x}
        )
        for x in year_1_archive_keys
    ]
    further_gms = [
        s3.generate_presigned_url(
            "get_object", Params={"Bucket": "eshc-bucket", "Key": x}
        )
        for x in further_archive_keys
    ]
    ...
    counter2 = (
        list(range(29, 39)) + [38] + list(range(39, 61)) + [60] + list(range(61, 73))
    )
    further_gms = zip(further_gms, [x[8:] for x in further_archive_keys], counter2)

    context = {
        "oldest_gms": oldest_gms,
    ...
    if request.method != "POST":
        ...
        if point_form.is_valid():
            messages.add_message(
                request, messages.SUCCESS, "Agenda point added successfully!"
            )
...
def upload_minutes(request, id):
    ...
    if request.method != "POST":
        ...
        if minutes_form.is_valid():
            ...
            messages.add_message(
                request, messages.SUCCESS, "Minutes uploaded successfully!"
            )
            return HttpResponseRedirect(reverse("home:agenda", args=(gm.id,)))
        else:
            messages.add_message(request, messages.WARNING, "Something went wrong?")
...
def groups(request):
    ...
    wgs_with_conv = [
        (wg, Group.objects.filter(name__endswith="Conv", name__startswith=wg.name)[0])
        for wg in wgs
    ]
...
def wsp(request):
    ...
    jobless = User.objects.filter(
        profile__current_member=True, role__assigned_to__isnull=True
    ).order_by("last_name", "first_name")
    wgs
    context = {"groups": wgs_and_roles, "jobless": jobless}
...
def wsp_subgroups(request):
    ...
    context = {"groups": sgs_and_roles, "jobless": jobless}
...
def check_info_share(request):
    ...
    if user.is_authenticated:
        if (
            user.first_name == ""
            or user.last_name == ""
            or user.profile.phone_number == ""
            or user.profile.perm_address == ""
        ):
            messages.add_message(
                request,
                messages.WARNING,
...
def check_leases(request):
    ...
    for lease in leases:
        if not Inventory.objects.filter(lease=lease).exists():
            messages.add_message(
                request, messages.WARNING, "One of your leases is missing an inventory!"
            )
            break
