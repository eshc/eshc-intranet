...
class WgUpdate(models.Model):
    ...
    group = models.ForeignKey(
        Group, limit_choices_to=models.Q(name__endswith="WG"), on_delete=models.CASCADE
    )
...
class LdapGroup(models.Model):
    def __str__(self):
        return self.ldap_cn.partition(",")[0].rpartition("=")[
            2
        ]  # extract from cn=*NAME*,ou=junk
...
class Role(models.Model):
    ...
    past_holders = models.ManyToManyField(
        User, verbose_name="Past holders", related_name="past_roles", blank=True
    )
...
class Room(models.Model):
    ...
    current_occupant = models.OneToOneField(
        User, null=True, on_delete=models.SET_NULL, unique=True, blank=True
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["roomno", "flat"], name="no_duplicate_rooms_in_flat"
            )
        ]
