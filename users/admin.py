...
class UserResource(resources.ModelResource):
    class Meta:
        ...
        fields = (
            ...
            "email",
            "profile__ref_number",
            "profile__current_member",
            "profile__share_received",
            "username",
        )
...
class UserAdmin(ImportExportModelAdmin):
    ...
    action_mark_members_current.short_description = (
        "Mark selected members as currently residing and active"
    )
    ...
    def action_mark_members_left(self, request, queryset):
        ...
        self.message_user(
            request,
            "%s member(s) marked as having left the co-op and inactive"
            % (rows_updated,),
        )

    action_mark_members_left.short_description = (
        "Mark selected members as having left the co-op and inactive"
    )
    ...
    action_give_share.short_description = (
        "Mark selected members as having paid the share"
    )
    ...
    def action_mark_current_read_lease(self, request, queryset):
        ...
        for usr in queryset.select_related("profile"):
            ...
            valid_lease = (
                Lease.objects.filter(
                    user=usr, start_date__lte=now, end_date__gte=now
                ).count()
                > 0
            )
        ...
        self.message_user(
            request,
            "%s (out of %s) members updated: %s new members, %s members deactivated"
            % (new_current + new_ex, scanned, new_current, new_ex),
        )

    action_mark_current_read_lease.short_description = (
        "Update selected members' active status based on signed lease"
    )

    actions = [
        "action_mark_members_current",
        "action_mark_members_left",
        "action_give_share",
        "action_remove_share",
