...
def index(request):
    ...
    for question in past_questions:
        ...
        for choice in Choice.objects.filter(question=question):
            vote_count.append(
                [
                    choice.choice_text,
                    Vote.objects.filter(question=question)
    ...
    for question in open_questions:
        current_questions.append(
            [question, Vote.objects.filter(question=question).count()]
        )
...
def detail(request, pk):
    ...
    if not (Vote.objects.filter(user=request.user, question=question).__nonzero__()):
        messages.add_message(
            request, messages.INFO, "You have not voted on this proposal."
        )
    ...
    if request.GET:
        ...
        if Vote.objects.filter(user=request.user, question=question).__nonzero__():
            ...
            messages.add_message(
                request,
                messages.WARNING,
                "You have already cast a vote. You voted {}".format(
                    v.choice.choice_text
                ),
            )

        else:
            v, created = Vote.objects.get_or_create(
                choice=choice_clicked[0], user=request.user, question=question
            )
            if created:
    ...
    for choice in choices:
        votes[choice.choice_text] = Vote.objects.filter(
